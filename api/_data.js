...
function normalisiereTeamSchluessel(name) {
    return normalizeText(name).replace(/[^a-z0-9]+/g, "");
}
function teamZuMannschaft(team) {
...
function istGruppenPhase(stage, gruppe) {
    return Boolean(gruppe) || /GROUP/i.test(stage ?? "");
}
function formatiereAbschnittName(abschnitt) {
    if (!abschnitt)
        return "Gruppenphase";
    const normalisiert = abschnitt.trim().toUpperCase();
    const gruppe = normalisiert.match(/^GROUP[_\s-]?([A-L])$/);
    if (gruppe)
        return `Gruppe ${gruppe[1]}`;
    if (normalisiert.includes("LAST_32") || normalisiert.includes("ROUND_OF_32"))
        return "Sechzehntelfinale";
    if (normalisiert.includes("LAST_16") || normalisiert.includes("ROUND_OF_16"))
        return "Achtelfinale";
    if (normalisiert.includes("QUARTER"))
        return "Viertelfinale";
    if (normalisiert.includes("SEMI"))
        return "Halbfinale";
    if (normalisiert.includes("THIRD") || normalisiert.includes("BRONZE"))
        return "Spiel um Platz 3";
    if (normalisiert.includes("FINAL"))
        return "Finale";
    if (normalisiert.includes("GROUP"))
        return "Gruppenphase";
    return abschnitt
        .replace(/_/g, " ")
        .toLowerCase()
...
function istEchteMannschaft(team) {
...
function istSichtbaresSpiel(spiel) {
    return !spiel.istKoSpiel || Boolean(spiel.istFixiert);
}
function matchZuSpiel(match) {
...
function istHeutigesDatum(a, b) {
    return a.toDateString() === b.toDateString();
}
let spieleCache;
let standingsCache;
const CACHE_MS = 60_000;
export async function ladeAlleSpiele() {
    if (spieleCache && Date.now() - spieleCache.zeit < CACHE_MS)
        return spieleCache.daten;
    const daten = await ladeJson(`/competitions/${WETTBEWERBS_CODE}/matches`);
...
export async function ladeStandings() {
    if (standingsCache && Date.now() - standingsCache.zeit < CACHE_MS)
        return standingsCache.daten;
    const daten = await ladeJson(`/competitions/${WETTBEWERBS_CODE}/standings`);
...
async function ladeGruppenAusSpielplan() {
    const spiele = await ladeAlleSpiele();
    const gruppen = new Map();
    for (const spiel of spiele) {
        if (spiel.istKoSpiel || !istEchteMannschaft(spiel.heimTeam) || !istEchteMannschaft(spiel.gastTeam))
            continue;
        const gruppe = gruppen.get(spiel.gruppe) ?? new Map();
...
function leereTabellenZeile(team, platz) {
    return {
        platz,
        team,
        spiele: 0,
...
function vervollstaendigeGruppen(standingsGruppen, spielplanGruppen) {
    const gruppen = new Map();
...
export async function ladeSpieleMitFilter(bereich, filter) {
    ...
    if (bereich === "heute")
        gefilterteSpiele = sichtbareSpiele.filter((spiel) => istHeutigesDatum(new Date(spiel.anpfiff), jetzt));
    else if (bereich === "naechste")
        gefilterteSpiele = sichtbareSpiele.filter((spiel) => new Date(spiel.anpfiff).getTime() >= jetzt.getTime()).slice(0, 6);
    else if (bereich === "live")
        gefilterteSpiele = sichtbareSpiele.filter((spiel) => spiel.status === "IN_PLAY" || spiel.status === "PAUSED");
    else if (bereich === "favoriten")
        gefilterteSpiele = sichtbareSpiele.slice(0, 8);
    if (filter === "heute")
        gefilterteSpiele = gefilterteSpiele.filter((spiel) => istHeutigesDatum(new Date(spiel.anpfiff), jetzt));
    if (filter === "naechste")
        gefilterteSpiele = gefilterteSpiele.filter((spiel) => new Date(spiel.anpfiff).getTime() >= jetzt.getTime()).slice(0, 6);
    if (filter === "live")
        gefilterteSpiele = gefilterteSpiele.filter((spiel) => spiel.status === "IN_PLAY" || spiel.status === "PAUSED");
    if (filter === "favoriten")
        gefilterteSpiele = gefilterteSpiele.slice(0, 8);
    return gefilterteSpiele;
}
export async function ladeSpielNachId(spielId) {
    const alleSpiele = await ladeAlleSpiele();
    return alleSpiele.find((spiel) => spiel.id === spielId);
}
function erzeugeIcsDate(datum) {
    return datum.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
}
export function baueIcsInhalt(spiel) {
